...
def filter_late_crc_update(
    ...
    if "decision_id" in update:
        output["decision_id"] = update["decision_id"]

    if "rnti" in update:
        output["rnti"] = rnti_hex(update["rnti"])

    target_slot = update.get("target_pusch_slot")
    if isinstance(target_slot, dict):
        output["target_pusch_slot"] = {
            key: target_slot[key]
            for key in ("hyper_sfn", "sfn", "slot_index")
            if key in target_slot
        }

    for key in ("harq_id", "crc_status"):
        if key in update:
