...
#include "phy/upper/phy_acceleration_prach_buffer_factory.h"
...
static std::string                 detector_backend  = "cpu";
static bool                        use_device_buffer = false;
...
static void usage(const char* prog)
{
  ...
  fmt::print("\t-W Warm-up repetitions per test case [Default {}]\n", nof_warmups);
  ...
  fmt::print("\t-b Detector backend: cpu, gpu, auto [Default {}]\n", detector_backend);
  fmt::print("\t-d Use a CUDA-visible PRACH buffer when available [Default {}]\n", use_device_buffer);
...
{
  int opt = 0;
  while ((opt = getopt(argc, argv, "R:W:p:f:z:b:dh")) != -1) {
    switch (opt) {
      case 'R':
        nof_repetitions = std::strtol(optarg, nullptr, 10);
        break;
      case 'W':
        nof_warmups = std::strtol(optarg, nullptr, 10);
        break;
      case 'p':
      ...
      case 'z':
        ...
        break;
      case 'b':
        detector_backend = optarg;
        if ((detector_backend != "cpu") && (detector_backend != "gpu") && (detector_backend != "auto")) {
          fmt::print("Invalid backend '{}'. Valid backends are: cpu, gpu, auto.\n", detector_backend);
          std::exit(-1);
        }
        break;
      case 'd':
        use_device_buffer = true;
        break;
      case 'h':
      default:
        usage(argv[0]);
...
static prach_detector_factory& get_detector_factory()
{
  ...
  if (detector_backend == "cpu") {
    prach_det_factory = create_prach_detector_factory_sw(dft_proc_factory, prach_gen_factory);
  } else {
    prach_det_factory = create_prach_detector_factory_accelerated(
        dft_proc_factory, prach_gen_factory, {}, detector_backend == "gpu" ? "enabled" : "auto");
  }
...
static std::unique_ptr<prach_buffer> create_buffer(prach_format_type format, unsigned nof_antennas)
{
  ...
  if (is_long_preamble(format)) {
    buffer = create_phy_acceleration_prach_buffer_long(nof_antennas, 1, use_device_buffer);
  } else {
    buffer = create_phy_acceleration_prach_buffer_short(nof_antennas, 1, 1, use_device_buffer);
  }
...
int main(int argc, char** argv)
{
  ...
  for (const prach_detector::configuration& config : configurations) {
    ...
    for (uint64_t warmup = 0; warmup != nof_warmups; ++warmup) {
