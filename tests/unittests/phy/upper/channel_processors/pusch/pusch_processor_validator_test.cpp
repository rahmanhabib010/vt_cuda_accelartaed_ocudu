...
const std::vector<test_case_t> pusch_processor_validator_test_data = {
    ...
    {[] {
       pusch_processor::pdu_t pdu = base_pdu;
       pdu.rx_ports.clear();
       for (unsigned i_port = 0, i_port_end = pusch_constants::MAX_NOF_RX_PORTS + 1; i_port != i_port_end; ++i_port) {
         pdu.rx_ports.emplace_back(i_port);
       }
       return pdu;
     },
     R"(The number of receive ports \(i\.e\., 9\) exceeds the maximum number of receive ports \(i\.e\., 8\)\.)"},
    {[] {
       pusch_processor::pdu_t pdu = base_pdu;
       pdu.bwp_size_rb            = 1;
       return pdu;
    ...
    {[] {
       pusch_processor::pdu_t pdu = base_pdu;
       pdu.uci.nof_harq_ack       = uci_constants::MAX_NOF_HARQ_BITS + 1;
       return pdu;
     },
     R"(UCI field sizes in bits \(1707, 1\), exceed the maximum bit sizes, i\.e\., 1706 and 1706\.)"},
    {[] {
       pusch_processor::pdu_t pdu = base_pdu;
       pdu.uci.nof_csi_part1      = uci_constants::MAX_NOF_CSI_PART1_OR_PART2_BITS + 1;
       return pdu;
     },
     R"(UCI field sizes in bits \(0, 1707\), exceed the maximum bit sizes, i\.e\., 1706 and 1706\.)"},
};
