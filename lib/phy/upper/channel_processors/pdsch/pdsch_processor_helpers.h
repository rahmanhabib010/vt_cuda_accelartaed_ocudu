...
/// \brief Appends PT-RS reserved REs to a PDSCH reserved pattern list, if PT-RS is configured.
inline void pdsch_reserve_ptrs_re(re_pattern_list& reserved, const pdsch_processor::pdu_t& pdu, const crb_bitmap& rb_mask)
{
...
inline unsigned pdsch_compute_nof_data_re(const pdsch_processor::pdu_t& pdu)
{
  ...
  pdsch_reserve_ptrs_re(reserved, pdu, crb_mask);
