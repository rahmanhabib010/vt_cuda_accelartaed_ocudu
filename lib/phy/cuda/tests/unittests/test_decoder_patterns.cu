// Test decoder with different bit patterns
#include "ocudu_phy_cuda.h"
#include <cstdio>
#include <cstdlib>
#include <vector>

int test_pattern(int Z, uint32_t pattern, const char* name) {
    const int bg = 2;
    const int Kb = 10;
    const int K = Kb * Z;
    ...
    printf("  %-12s: Errors=%3d/%d %s\n", name, errors, K, errors == 0 ? "PASS" : "FAIL");
...
int main() {
    ...
    for (int Z : test_z) {
        printf("\n=== Z=%d ===\n", Z);
        test_pattern(Z, 0x00000000, "All zeros");
        test_pattern(Z, 0xFFFFFFFF, "All ones");
        test_pattern(Z, 0x00000001, "Single bit");
        test_pattern(Z, 0x12345678, "Random 1");
        test_pattern(Z, 0xAAAAAAAA, "Alternating");
        test_pattern(Z, 0x55555555, "Alt inverse");
    }
