// Test to find Z boundary where decoder fails
#include "ocudu_phy_cuda.h"
#include <cstdio>
#include <cstdlib>
#include <vector>

int test_z_value(int Z) {
    const int bg = 2;
    const int Kb = 10;
    const int K = Kb * Z;
...
int main() {
    ...
    printf("Testing decoder at different Z values (BG2):\n");
    printf("%-8s %-8s %-10s %-10s\n", "Z", "K", "Errors", "Status");
    printf("------------------------------------------------\n");

    // Test Z values from lifting set 4 (base=9): 9, 18, 36, 72, 144, 288
    int test_values[] = {9, 18, 36, 72, 32, 48, 64};

    for (int z : test_values) {
        int errors = test_z_value(z);
        int K = 10 * z;
        printf("%-8d %-8d %-10d %-10s\n", z, K, errors, errors == 0 ? "PASS" : "FAIL");
    }
